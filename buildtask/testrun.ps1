...
param($TestResultPath,$OutputFolder,$JsonMapping,$InputType)
    ...
    Start-Process  -FilePath ".\Microsoft.DX.JavaTestBridge.UnitTestGenerator.exe" -ArgumentList "AutomatedTestAssembly `"$jsonMapping`" $testResultPath" 
    ...
    if($LASTEXITCODE -eq 0){
        ...
        Start-Process -FilePath ".\Microsoft.DX.JavaTestBridge.VSTS.exe" -ArgumentList "$vsoAccountName $projectName AutomatedTestAssembly.dll $username $password" 
        if($LASTEXITCODE -eq 0)
        {
